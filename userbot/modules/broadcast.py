...
loggy_grp = BOTLOG_CHATID


@register(outgoing=True, pattern=r"^\.badd ?(.*)")
async def _(event):
    ...
    if not already_added(input_chnnl):
        ...
        await bot.send_message(loggy_grp, f"Added {input_chnnl} To DB")


@register(outgoing=True, pattern=r"^\.brm ?(.*)")
async def _(event):
    input_chnnl = event.pattern_match.group(1)
    ...
    if already_added(input_chnnl):
        ...
        await bot.send_message(loggy_grp, f"Removed {input_chnnl} From DB")
...
async def _(event):
    ...
    if hmm and hmm.media:
        ok = await bot.download_media(hmm.media, sedpath)
        for channelz in all_chnnl:
            try:
                await bot.send_file(int(channelz.chat_id), file=ok, caption=hmm.text)
                total_count += 1
        ...
        for channelz in all_chnnl:
            try:
                await bot.send_message(int(channelz.chat_id), hmm.text)
                total_count += 1
            except Exception as e:
                total_errors += 1
                errorno += f"{e} \n"
...
@register(outgoing=True, pattern=r"^\.bforward")
async def _(event):
    all_chnnl = get_all_chnnl()
    if len(all_chnnl) == 0:
        await event.edit("No Channel Or Group Found On Database. Please Check Again")
    ...
    await bot.send_message(
        loggy_grp,
        f"Forward in {total_count} And Failed In {total_errors} And Total Channel In Db is {total_chnnl}",
    )


@register(outgoing=True, pattern=r"^\.bstat ?(.*)")
async def _(event):
